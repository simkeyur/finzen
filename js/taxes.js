...
class TaxCalculator {
    ...
    setupEventListeners() {
        ...
        // Reset button (optional - FinZen uses settings modal)
        if (this.resetBtn) {
            this.resetBtn.addEventListener('click', () => this.resetApp());
        }
        
        // Install button (optional - PWA feature)
        if (this.installBtn) {
            this.installBtn.addEventListener('click', () => this.handleInstallClick());
        }
        
        // iOS Modal (optional - not used in FinZen)
        if (this.modalClose) {
        ...
        // Listen for PWA install prompt (only if install button exists)
        if (this.installBtn) {
    ...
    checkiOSInstall() {
        if (!this.installBtn) return; // Skip if install button doesn't exist
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // PWA Install Prompt (optional, only if install button exists)
    let deferredPrompt;
